...
namespace WhatsAppPort
{
    public partial class frmUserChat : Form
    {
        ...
        public void TrasferMsj(string msj) {
            this.txtBxSentText.Text = msj;
            if (this.txtBxSentText.Text.Length == 0)
                return;
            WhatSocket.Instance.SendMessage(this.user.WhatsUser.GetFullJid(), txtBxSentText.Text);
            this.AddNewText(this.user.UserName, txtBxSentText.Text);
        ...
        public void TrasferMessageImage(byte [] image) {
            WhatsAppApi.ApiBase.ImageType typeimage = WhatsAppApi.ApiBase.ImageType.JPEG;
            WhatSocket.Instance.SendMessageImage(this.user.WhatsUser.GetFullJid(),image ,typeimage);
        }
